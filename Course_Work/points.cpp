...
Point geo_to_rect2(Point &point)
{
    Point result_point = {0, 0 , 0};

    double radius_curvature = (EQUATOR_RAD + 1000000) / sqrt(1.0 - \
       SQR_ECCENTRICITY * sin(to_rad(point.x)) * sin(to_rad(point.x)));
    ...
    //result_point[2] = (radius_curvature * (EQUATOR_RAD * EQUATOR_RAD / POLAR_RAD / POLAR_RAD)
    //                   + point[2]) * sin(to_rad(point[0]));
...
void convert_coordinates(Point &point)
{
    double x0 = point.x;
    double y0 = point.y;
    double z0 = point.z;
    //point[2] = sqrt(2) / 2 * point[2];
    //point[0] = point[0] - point[2];
    //point[1] = point[2] - point[1];
    //point[0] *= 0.5;
    //point[1] *= 0.5;

    //point[0] = x0 * cos(to_rad(60)) + z0 * sin(to_rad(60));
    //point[2] = -x0 * sin(to_rad(60)) + z0 * cos(to_rad(60));

    //turn_around_x(point, 90);
    //turn_around_y(point, 30);
    /*
    double x0 = point[0];
    double y0 = point[1];
    point[0] = x0 - (x0 - point[0]);
    point[1] = y0 - (y0 - point[1]);
    */
}
