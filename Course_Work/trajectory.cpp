...
{
    double min = parabola_equation_result(coef_a, 0, -25);

    for (double j = -24; j <= 25; j++)
    {
        double value = parabola_equation_result(coef_a, 0, j);
        if (min > value)
        {
            qDebug() << "MIN " << min << " VALUE " << value;
            min = value;
        }
    }
    return abs(min);
...
void Trajectory::build2()
{
    Point start_coords =this->start_point;
    qDebug() << "ST " << start_coords.x << "  " << start_coords.y;
    Point end_coords = this->end_point;
    qDebug() << "FIN " << end_coords.x << "  " << end_coords.y;
    ...
    for (int i = 0; i <= TRAJECTORY_POINTS; i++)
    {
        ...
        point.z = (parabola_equation_result(coef_a, 0, i - TRAJECTORY_POINTS / 2) + fix_h) / SCALE;
